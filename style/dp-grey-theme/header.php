...
if (!$AppUI) {
	// AppUI not available, set defaults
...
<head>
	...
	<meta name="Version" content="<?php echo ($AppUI ? $AppUI->getVersion() : '1.0');?>" />
...
?>
	...
	<form name="frmHeaderSearch" action="?m=smartsearch"  method="post">
		...
		<input class="button" type="submit" value="<?php echo $AppUI ? $AppUI->_('Search') : 'Search'?>" />
	</form>
	<?php endif; ?>
	</th>
	...
	foreach ($nav as $module) {
		if (getPermission($module['mod_directory'], 'access')) {
			$links[] = ('<a href="?m=' . dPformSafe($module['mod_directory'], DP_FORM_URI) . '">' 
						. ($AppUI ? $AppUI->_($module['mod_ui_name']) : $module['mod_ui_name']) . '</a>');
		}
	}
	echo implode(' | ', $links);
	echo "\n";
...
<tr>
	<td>
		...
		<tr>
			<td width="100%"><?php 
	echo ($AppUI ? ($AppUI->_('Welcome') . ' ' . $AppUI->user_first_name . ' ' . $AppUI->user_last_name) : 'Welcome Guest'); 
?></td>
			<td nowrap="nowrap">
				<?php echo dPcontextHelp('Help'); ?> | 
				<a href="./index.php?m=admin&amp;a=viewuser&amp;user_id=<?php 
echo $AppUI ? $AppUI->user_id : 0; ?>"><?php echo $AppUI ? $AppUI->_('My Info') : 'My Info';?></a> | <?php
	if (getPermission('tasks', 'access')) {
?>
				<a href="./index.php?m=tasks&amp;a=todo"><b><?php echo $AppUI ? $AppUI->_('Todo') : 'Todo'; 
?></b></a> | <?php
	}
...
echo $now->format(FMT_TIMESTAMP_DATE);?>"><?php echo $AppUI ? $AppUI->_('Today') : 'Today';?></a> | <?php 
	}
?>
				<a href="./index.php?logout=-1"><?php echo $AppUI ? $AppUI->_('Logout') : 'Logout';?></a>
			</td>
		</tr>
		</table>
	</td>
